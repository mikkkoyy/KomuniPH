 ...
 */
const ALIAS_PATTERN = /^[A-Za-z0-9_.-]{1,50}$/;

/**
 * Fetch the full profile row for a user, with username joined.
 */
 ...
 *
 * Editable fields: display_name, bio, alias, alias_enabled.
 * No database migration required — all columns already exist.
 */
export async function handleUpdateProfile(req, res, user) {
  ...
  try {
    if (!body || typeof body !== 'object' || Array.isArray(body)) {
      return errorResponse(res, 422, 'No valid profile fields to update');
    }

    const ALLOWED = ['display_name', 'bio', 'alias', 'alias_enabled'];
    ...
    if (body.bio !== undefined) {
      if (body.bio !== null && typeof body.bio !== 'string') {
        ...
        const bio = body.bio === null ? '' : body.bio.trim();
        if (bio.length > 500) {
    ...
    if (body.alias !== undefined) {
      if (body.alias !== null && typeof body.alias !== 'string') {
        ...
        const alias = body.alias === null ? '' : body.alias.trim();
        if (alias.length === 0) {
          updates.alias = null;
        } else if (alias.length < 3 || alias.length > 50) {
          errors.push('Alias must be 3-50 characters');
        } else if (!/^[a-zA-Z0-9_]+$/.test(alias)) {
          errors.push('Alias may contain only letters, numbers, and underscore');
        } else {
          updates.alias = alias;
        }
      }
    ...
    if (body.alias_enabled !== undefined) {
      if (typeof body.alias_enabled !== 'boolean') {
        errors.push('Alias visibility must be true or false');
      } else {
        updates.alias_enabled = body.alias_enabled ? 1 : 0;
      }
    }
    ...
    const values = [...Object.values(updates), user.sub];
    execute(
...
/**
 * Validate and normalize profile edit fields.
 * Returns { values } on success or { error } on validation failure.
 */
function validateProfileUpdate(body) {
  const values = {};
  const hasDisplayName = body.display_name !== undefined;
  const hasBio = body.bio !== undefined;
  const hasAlias = body.alias !== undefined;

  if (!hasDisplayName && !hasBio && !hasAlias) {
    return { error: 'At least one of display_name, bio, or alias is required' };
  }
  ...
  if (hasAlias) {
    ...
    if (alias.length > 50) {
      return { error: 'Alias must be less than 50 characters' };
    }
    // Predictable policy: letters, numbers, spaces, underscore, hyphen, dot.
    if (!/^[A-Za-z0-9_.\- ]+$/.test(alias)) {
      return { error: 'Alias may only contain letters, numbers, spaces, underscores, hyphens, and dots' };
    }
    // Normalize: trim + collapse consecutive whitespace.
    values.alias = alias.replace(/\s+/g, ' ');
  }
...
export async function handleUpdateProfile(req, res, user) {
  try {
    ...
    try {
      ...
      return errorResponse(res, 422, 'Invalid JSON body');
    }

    if (!body || typeof body !== 'object' || Array.isArray(body)) {
      return errorResponse(res, 422, 'Invalid request body');
    }

    const { values, error } = validateProfileUpdate(body);
    if (error) {
      return errorResponse(res, 422, error);
    }

    // Alias uniqueness (case-insensitive, excluding own profile).
    if (values.alias !== undefined) {
      const taken = queryOne(
        `SELECT p.user_id FROM profiles p WHERE LOWER(p.alias) = LOWER(?) AND p.user_id != ? LIMIT 1`,
        [values.alias, user.sub]
      );
      if (taken) {
    ...
    const existing = queryOne('SELECT id FROM profiles WHERE user_id = ?', [user.sub]);
    if (!existing) {
      return errorResponse(res, 404, 'Profile not found');
    }
    ...
    if (values.display_name !== undefined) {
      ...
      params.push(values.display_name);
    }
    if (values.bio !== undefined) {
      sets.push("bio = ?");
      params.push(values.bio);
    }
    if (values.alias !== undefined) {
      sets.push("alias = ?");
      params.push(values.alias);
    }
    sets.push("updated_at = datetime('now')");
    params.push(user.sub);

    try {
      execute(`UPDATE profiles SET ${sets.join(', ')} WHERE user_id = ?`, params);
    } catch (dbErr) {
      console.error('[PROFILE] Update profile error:', dbErr.message);
...
export async function handleUpdateProfile(req, res, user) {
  try {
    ...
    updated.alias_enabled = Boolean(updated.alias_enabled);
