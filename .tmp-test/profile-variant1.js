...
async function processPhotoUpload(fileInfo, user) {
  ...
  let metadata = { format: 'skip' }; // VARIANT1: input metadata() disabled for bisect
  try {
    // metadata = await sharp(data).metadata();
    const resizeProbe = await sharp(data)
      .resize({ width: 1, height: 1, fit: 'inside', withoutEnlargement: true })
      .toBuffer();
    if (!resizeProbe || resizeProbe.length === 0) throw new Error('empty probe');
  } catch (metaErr) {
