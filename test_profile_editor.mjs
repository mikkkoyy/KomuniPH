...
try {
  ...
  for (const width of [390, 768, 1280, 1366]) {
    ...
    assert.equal(await page.$('#photo-gallery-upload'), null);
  ...
  await page.click('.editor-heading a');
  await page.waitForSelector('#profile-frame', { visible: true });
  assert.equal(await page.$('#profile-create-album-btn'), null);
  assert.equal(await page.$('#photo-gallery-upload'), null);
  ...
  await page.click('#create-album-btn');
  await field('create-album-name', 'Editor Regression Album');
  await page.click('#create-album-submit');
  await page.waitForFunction(() => document.getElementById('gallery-albums-grid').textContent.includes('Editor Regression Album'));
